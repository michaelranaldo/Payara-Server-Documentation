...
On *EMBEDDED* mode, the OpenMQ broker is executed as part of the same JVM process the application server is running latexmath:[$Only works with Payara Server Full Edition$]. Considering this, the configuration property must be set as part of the JVM settings of the domain. To configure the JMS service using the respective `asadmin` command you can do this:
