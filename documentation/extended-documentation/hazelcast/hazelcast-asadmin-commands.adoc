= Hazelcast Asadmin Command Reference

== `set-hazelcast-configuration`

*Usage:* `asadmin> set-hazelcast-configuration --enabled=true`

*Aim:* Enables Hazelcast, which is disabled by default, as well as allowing all
its configuration. This command requires the admin server to be running on port
...
*Aim:* Creates a new Payara Server instance with the ability to set instance
settings, configure Hazelcast and set the persistence types. A very long
command, suitable for scripted installs (i.e. `pre-boot-` and `post-boot-commands.txt`).
...
List the current members of the DAS' Hazelcast cluster in a
formatted table, with extra properties (compared to `list-hazelcast-members`).
...
*Aim:* List the hazelcast members as a targetable array at an instance level.
